    ...
    , ui(new Ui::onb1)
{
    ...
    ui->tw->setHorizontalHeaderItem(2, new QTableWidgetItem("Адрес"));
    ui->tw->setHorizontalHeaderItem(3, new QTableWidgetItem("сайт"));
...
void onb1::selectAll()
{
    ...
    QString sqlstr = "SELECT * FROM faculties ORDER BY f_id";
    ...
    while (query.next())
    {
        ...
        QTableWidgetItem *idItem = new QTableWidgetItem(query.value("f_id").toString());
        ui->tw->setItem(rowCount, 0, idItem);

        // Заполняем значениями из базы данных
        ui->tw->setItem(rowCount, 1, new QTableWidgetItem(query.value("f_name").toString()));
        ui->tw->setItem(rowCount, 2, new QTableWidgetItem(query.value("f_address").toString()));
        ui->tw->setItem(rowCount, 3, new QTableWidgetItem(query.value("f_website").toString()));
...
void onb1::add()
{
    ...
    QString sqlstr = "INSERT INTO faculties(f_name, f_address, f_website) VALUES (?, ?, ?)"; // Формируем строку SQL-запроса для вставки данных в таблицу
...
void onb1::remove()
{
    ...
    QString sqlstr = "DELETE FROM faculties WHERE f_id = '" + pu1 + "'";
    // Выполняем запрос
    if (!query.exec(sqlstr))
    {
...
void onb1::edit()
{
    ...
    QString sqlstr = "UPDATE faculties SET f_name = ?, f_address = ?, f_website = ? WHERE f_id = ?";
...
void onb1::generateReportRequest() {
    ...
    QString reportText = "Запрос на формирование отчета - о Факультетах:\n\n";
    reportText += "Уважаемые коллеги,\n\n";
    reportText += "Подготовлен отчет о Факультетах:\n";
    ...
    QString fileName = "Faculties_report_request.txt"; // Имя файла для сохранения отчета
